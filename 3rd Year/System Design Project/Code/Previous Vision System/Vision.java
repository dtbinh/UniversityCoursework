...
public class Vision extends WindowAdapter {
    ...
    private void initFrameGrabber(String videoDevice, int inWidth, int inHeight, int channel,
        ...
        frameGrabber.setCaptureCallback(new CaptureCallback() {
            ...
            public void nextFrame(VideoFrame frame) {
                ...
                if (worldState.getBackground()) {
                    try {
                        File sampleImage = new File("TestImage.jpeg");
                        ImageIO.write(frameImage, "jpeg", sampleImage);
                        worldState.setBackground(false);
                    } catch (Exception e) {
                        //We'll try again
                        worldState.setBackground(true);
                    }
                }
    ...
    public FrameGrabber fetchBackground() {
        return frameGrabber;
    }
...
//         ArrayList<Integer> blueFilteredY = bo.ypoints;
        ...
        /*
         * Check the distance between blue and green first
         */
        boolean blueIsFirst;
        ...
        ArrayList<Integer> newBlueXPoints = new ArrayList<Integer>();
        ArrayList<Integer> newBlueYPoints = new ArrayList<Integer>();
        ArrayList<Integer> newYellowXPoints = new ArrayList<Integer>();
        ArrayList<Integer> newYellowYPoints = new ArrayList<Integer>();
        ...
        if (worldState.getDetectTwo()) {
            ...
            /*
             * If blue is the first robot, the distance is going 
             * to be smaller for one of the green plates.
             */

            if (distance(blueX, blueY, greenfirstx, greenfirsty) < distance(blueX, blueY, greensecondx, greensecondy)) {
                blueIsFirst = true;

                //Check the boundaries of the green plate only for blue pixels
                if (firstRobotMaxX != 0 && firstRobotMinX != 0 && firstRobotMaxY != 0 && firstRobotMinY != 0) {
                    for (int x = 0; x < (firstRobotMaxX - firstRobotMinX); x++) {
                        for (int y = 0; y < (firstRobotMaxY - firstRobotMinY); y++) {
                            float hsbvals[] = new float[3];
                            Color c = new Color(image.getRGB(firstRobotMinX + x, firstRobotMinY + y));
                            Color.RGBtoHSB(c.getRed(), c.getGreen(), c.getBlue(), hsbvals);
                            if (isBlue(c, hsbvals)) {
                                newBlueX += firstRobotMinX + x;
                                newBlueY += firstRobotMinY + y;
                                newNumBlue++;
                                newBlueXPoints.add(firstRobotMinX + x);
                                newBlueYPoints.add(firstRobotMinY + y);
                            }
                        }
                    }
                }

                //Check the boundaries of the green plate only for yellow pixels
                if (secondRobotMaxX != 0 && secondRobotMinX != 0 && secondRobotMaxY != 0 && secondRobotMinY != 0) {
                    for (int x = 0; x < (secondRobotMaxX - secondRobotMinX); x++) {
                        for (int y = 0; y < (secondRobotMaxY - secondRobotMinY); y++) {
                            float hsbvals[] = new float[3];
                            Color c = new Color(image.getRGB(secondRobotMinX + x, secondRobotMinY + y));
                            Color.RGBtoHSB(c.getRed(), c.getGreen(), c.getBlue(), hsbvals);
                            if (isYellow(c, hsbvals)) {
                                newBlueX += firstRobotMinX + x;
                                newBlueY += firstRobotMinY + y;
                                newNumBlue++;
                                newYellowXPoints.add(secondRobotMinX + x);
                                newYellowYPoints.add(secondRobotMinY + y);
                            }
                        }
                    }
                }
                    ...
                    for (int x = 0; x < (firstRobotMaxX - firstRobotMinX); x++) {
                        for (int y = 0; y < (firstRobotMaxY - firstRobotMinY); y++) {
                            ...
                            if (isYellow(c, hsbvals)) {
                                ...
                                newBlueY += firstRobotMinY + y;
                                newNumBlue++;
                                newYellowXPoints.add(firstRobotMinX + x);
                                newYellowYPoints.add(firstRobotMinY + y);
                            }
                        }
                    }
                }
                    ...
                    for (int x = 0; x < (secondRobotMaxX - secondRobotMinX); x++) {
                        for (int y = 0; y < (secondRobotMaxY - secondRobotMinY); y++) {
                            ...
                            if (isBlue(c, hsbvals)) {
                                ...
                                newBlueY += firstRobotMinY + y;
                                newNumBlue++;
                                newBlueXPoints.add(secondRobotMinX + x);
                                newBlueYPoints.add(secondRobotMinY + y);
                            }
                        }
                    }
                }
        ...
        } else { //We are testing only one plate
            blueIsFirst = false;
            try {
                firstRobotMaxX = Collections.max(greenXPoints);
                firstRobotMaxY = Collections.max(greenYPoints);
                firstRobotMinX = Collections.min(greenXPoints);
                firstRobotMinY = Collections.min(greenYPoints);
            } catch (Exception e) {
            ...
            if (firstRobotMaxX != 0 && firstRobotMinX != 0 && firstRobotMaxY != 0 && firstRobotMinY != 0) {
                for (int x = 0; x < (firstRobotMaxX - firstRobotMinX); x++) {
                    for (int y = 0; y < (firstRobotMaxY - firstRobotMinY); y++) {
                        ...
                        Color c = new Color(image.getRGB(firstRobotMinX + x, firstRobotMinY + y));
                        Color.RGBtoHSB(c.getRed(), c.getGreen(), c.getBlue(), hsbvals);
                        if (isBlue(c, hsbvals) || isYellow(c, hsbvals)) {
                            newBlueX += firstRobotMinX + x;
                            newBlueY += firstRobotMinY + y;
                            newNumBlue++;
                            newBlueXPoints.add(firstRobotMinX + x);
                            newBlueXPoints.add(firstRobotMinY + y);
                        }
                    }
                }
            }
        ...
        if (newNumYellow > 0) {
            newYellowX /= newNumYellow;
            newYellowY /= newNumYellow;
        }
        ...
        try {
            if (runAlternate == false) {
                blueOrientation = getGeometricOrientation(newBlueXPoints, newBlueYPoints, blue.getX(), blue.getY(), image, true);
...
//            }
        ...
        try {
            if (true != false) {
                yellowOrientation = getGeometricOrientation(newYellowXPoints, newYellowYPoints, yellow.getX(), yellow.getY(), image, false);
...
//            System.out.println("BlueX: " + blueoneX + " BlueY: " + blueoneY + " WhiteX: " + whiteoneX + " WhiteY: " + whiteoneY + " RedX: " + redoneX + " RedY: " + redoneY + " BlackX: " + blackoneX + " BlackY: " + blackoneY);
            ...
            if (clusterChecker(redoneX, redoneY, blackoneX, blackoneY, whiteoneX, whiteoneY, blueoneX, blueoneY, "blue")) {
                 ...
                 * average of the cluster for the extrema of the other.
                 * If you want to see this, use the vision tab. 
                 */
                frontPointX = blueoneX;
                frontPointY = blueoneY;
                backPointX = (redoneX + whiteoneX) / 2;
                backPointY = (redoneY + whiteoneY) / 2;
            }
            //Right facing
            if (clusterChecker(redoneX, redoneY, blackoneX, blackoneY, blueoneX, blueoneY, whiteoneX, whiteoneY, "white")) {
                ...
                backPointX = (blackoneX + blueoneX) / 3;
                backPointY = (blackoneY + blueoneY) / 3;
            }
            //Bottom facing
            if (clusterChecker(redoneX, redoneY, whiteoneX, whiteoneY, blueoneX, blueoneY, blackoneX, blackoneY, "black")) {
                ...
                backPointX = (redoneX + whiteoneX) / 3;
                backPointY = (redoneY + whiteoneY) / 3;
            }
            //Left facing
            ...
            /*
             * Display the information depending on which values are triggered in the GUI
             */
...
//                return findOrientation(xpoints, ypoints, meanX, meanY, image, false);
                ...
                if (worldState.getBAngle() || worldState.getYAngle()) {
