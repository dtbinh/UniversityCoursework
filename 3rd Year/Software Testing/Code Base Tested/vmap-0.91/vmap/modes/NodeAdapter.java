 ...
 */
/*$Id: NodeAdapter.java,v 1.21 2005/04/13 15:45:39 veghead Exp $ */
...
public abstract class NodeAdapter implements MindMapNode {
    ...
    private String title = ""; //Change this to vector in future for full graph support
    private String attachPath = ""; //Change this to vector in future for full graph support
...
//     protected String mLabel;
    ...
    protected LIPTypeName typename;
    ...
    protected NodeAdapter(VmapMain frame) {
	this.frame = frame;
        nodeId=frame.getController().getUniqueId();
    }

    protected NodeAdapter(Object userObject, VmapMain frame) {
	this.userObject = userObject;
	this.frame = frame;
    }

    protected NodeAdapter(String title, VmapMain frame) {
	this.title = title;
	this.frame = frame;
    }
...
//     public     void addReference(MindMapLink mindMapLink) { mNodeLinkVector.add(mindMapLink); };
    ...
    public void setAttach(String attachPath) {
...
//    // the meaning, at least not to me.
    ...
    /**
     * Get the node's primary icon
     * @returns first icon name or "noicon"
     */
    public String getPrimaryIcon() {
        String iconType="noicon";

        Vector icons = getIcons();

        if (icons.size()>=1) {
            iconType=((MindIcon)icons.get(0)).getName();
        }
        return iconType;
    }
    ...
    /**
     * Sets the LIP TypeName
     * @param liptype an IMS LIP typename
     */
    ...
    /**
     * gets the LIP TypeName
     * @return IMS LIP typename
     */
    ...
    /**
     * gets the LIP Identification
     * @return IMS LIP Identification class
     */
    ...
    /**
     * Sets the LIP Identification
     * @param liptype an IMS LIP Identification
     */
    ...
    /**
     * Sets or unsets visability
     * Only applies to clouds!
     * @param set true if visible, false if not.
     */
