...
public class GraphingData extends JPanel {
  ...
  protected void paintComponent(Graphics g) {
    ...
    for(int i = 0; i < data.length-1; i++) {
      ...
      double y1 = h - PAD - scale * (data[i][n_] * Math.log(data[i][n_]) * maxRatio / 1000);
      double x2 = PAD + (i+1)*xInc;
      // double y2 = h - PAD - scale*(Math.pow(data[i+1][n_], complexityDegree) * maxRatio);
      double y2 = h - PAD - scale * (data[i+1][n_] * Math.log(data[i+1][n_]) * maxRatio / 1000);
      g2.draw(new Line2D.Double(x1, y1, x2, y2));
    }
    ...
    for(int i = 0; i < data.length-1; i++) {
      double x1 = PAD + i*xInc;
      // double y1 = h - PAD - scale*(Math.pow(data[i][n_], complexityDegree) * maxRatio);
      double y1 = h - PAD - scale * (data[i][n_] * Math.log(data[i][n_]) * avgRatio / 1000);
      double x2 = PAD + (i+1)*xInc;
      // double y2 = h - PAD - scale*(Math.pow(data[i+1][n_], complexityDegree) * maxRatio);
      double y2 = h - PAD - scale * (data[i+1][n_] * Math.log(data[i+1][n_]) * avgRatio / 1000);
      g2.draw(new Line2D.Double(x1, y1, x2, y2));
    }
    
    // Draw legend
    g2.setStroke(new BasicStroke(2));
    s = "Shamos's algorithm runtimes";
    sw = (float)font.getStringBounds(s, frc).getWidth();
...
//    }
	...
	for (int i = 0; i < data.length; i++) {
	  if (data[i][this.school_] > max)
	    max = data[i][this.school_];
  ...
  public static void main(String[] args) {
    double[][] data = {
      {21, 10}, {14, 9}, {18, 04}, {03, 03}, {86, 72},
      {88, 14}, {74, 87}, {54, 77}, {61, 55}, {48, 60}, 
      {49, 36}, {38, 27}, {20, 18}, {23, 21}, {45, 02}
    };
    JFrame f = new JFrame();
