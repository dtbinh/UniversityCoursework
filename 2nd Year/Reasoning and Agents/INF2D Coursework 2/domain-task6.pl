...
:- multifile position/2, cleaned/2, dusterpos/3, has_duster/2, bin/2.
...
poss( dust(Room), S) :- 
	position(Room, S), not(cleaned(Room, S)),has_duster(_,S), bin(X,S), X<3, not(lift(Room)).
...
poss( empty(Room), S) :- 
	position(Room, S), not(lift(Room)), bin(X,S), not(X = 0).
