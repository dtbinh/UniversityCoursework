...
public final class InGameInputHandler extends InputHandler {
    ...
    private Element opponentMove(Element opponentMoveElement) {
        ...
        if (fromTile == null) {
            logger.warning("Ignoring opponentMove, no fromTile.");
            return null;
        }
        ...
        Unit unit = (Unit) getGame().getFreeColGameObjectSafely(opponentMoveElement.getAttribute("unit"));
        if (unit == null) {
        ...
        final String key = (getFreeColClient().getMyPlayer() == unit.getOwner()) ? ClientOptions.MOVE_ANIMATION_SPEED
    ...
    private Element setCurrentPlayer(Element setCurrentPlayerElement) {
        ...
        final Player currentPlayer = (Player) game.getFreeColGameObject(
                setCurrentPlayerElement.getAttribute("player"));
    ...
    private Element newTurn(Element newTurnElement) {
        ...
        if(getFreeColClient().getClientOptions().getBoolean(ClientOptions.ENDOFTURN_ALERT)) { 
    ...
    private Element newConvert(Element element) {
        ...
        String nation = Messages.getName(Specification.getSpecification().getNation(element.getAttribute("nation")));

        Element unitElement = (Element) element.getFirstChild();
        Unit convert = new Unit(getGame(), unitElement);
        tile.add(convert);

        ModelMessage message = new ModelMessage(convert, ModelMessage.MessageType.UNIT_ADDED, convert,
                "model.colony.newConvert", "%nation%", nation, "%colony%", colony.getName());
    ...
    private Element diplomacy(Element element) {
        ...
        agreement = new ShowNegotiationDialogSwingTask(message.getUnit(), message.getSettlement(), message
                .getAgreement()).select();
        if (agreement == null) {
            message.setReject();
        } else {
        ...
        if (goodsElement == null) {
            ...
            case ClientOptions.INDIAN_DEMAND_RESPONSE_ASK:
                accepted = new ShowConfirmDialogSwingTask("indianDemand.gold.text", "indianDemand.gold.yes",
                        "indianDemand.gold.no", "%nation%", Messages.getNationAsString(unit.getOwner()), "%colony%",
                        colony.getName(), "%amount%", String.valueOf(gold)).confirm();
                break;
            case ClientOptions.INDIAN_DEMAND_RESPONSE_ACCEPT:
                m = new ModelMessage(colony, ModelMessage.MessageType.ACCEPTED_DEMANDS, unit, "indianDemand.gold.text",
                        "%nation%", Messages.getNationAsString(unit.getOwner()), "%colony%", colony.getName(),
                        "%amount%", String.valueOf(gold));
                accepted = true;
                break;
            case ClientOptions.INDIAN_DEMAND_RESPONSE_REJECT:
                m = new ModelMessage(colony, ModelMessage.MessageType.REJECTED_DEMANDS, unit, "indianDemand.gold.text",
                        "%nation%", Messages.getNationAsString(unit.getOwner()), "%colony%", colony.getName(),
                        "%amount%", String.valueOf(gold));
                accepted = false;
                break;
            ...
            case ClientOptions.INDIAN_DEMAND_RESPONSE_ASK:
                if (goods.getType() == Goods.FOOD) {
                    accepted = new ShowConfirmDialogSwingTask("indianDemand.food.text", "indianDemand.food.yes",
                            "indianDemand.food.no", "%nation%", Messages.getNationAsString(unit.getOwner()),
                            "%colony%", colony.getName(), "%amount%", String.valueOf(goods.getAmount())).confirm();
                } else {
                    accepted = new ShowConfirmDialogSwingTask("indianDemand.other.text", "indianDemand.other.yes",
                            "indianDemand.other.no", "%nation%", Messages.getNationAsString(unit.getOwner()),
                            "%colony%", colony.getName(), "%amount%", String.valueOf(goods.getAmount()), "%goods%",
                            Messages.getName(goods)).confirm();
                }
                break;
            case ClientOptions.INDIAN_DEMAND_RESPONSE_ACCEPT:
                if (goods.getType() == Goods.FOOD) {
                    m = new ModelMessage(colony, ModelMessage.MessageType.ACCEPTED_DEMANDS, unit,
                            "indianDemand.food.text", "%nation%", Messages.getNationAsString(unit.getOwner()),
                            "%colony%", colony.getName(), "%amount%", String.valueOf(goods.getAmount()));
                } else {
                    m = new ModelMessage(colony, ModelMessage.MessageType.ACCEPTED_DEMANDS, unit,
                            "indianDemand.other.text", "%nation%", Messages.getNationAsString(unit.getOwner()),
                            "%colony%", colony.getName(), "%amount%", String.valueOf(goods.getAmount()), "%goods%",
                            Messages.getName(goods));
                }
                accepted = true;
                break;
            case ClientOptions.INDIAN_DEMAND_RESPONSE_REJECT:
                if (goods.getType() == Goods.FOOD) {
                    m = new ModelMessage(colony, ModelMessage.MessageType.REJECTED_DEMANDS, unit,
                            "indianDemand.food.text", "%nation%", Messages.getNationAsString(unit.getOwner()),
                            "%colony%", colony.getName(), "%amount%", String.valueOf(goods.getAmount()));
                } else {
                    m = new ModelMessage(colony, ModelMessage.MessageType.REJECTED_DEMANDS, unit,
    ...
    private Element monarchAction(Element element) {
        ...
        case RAISE_TAX:
            ...
            if (force) {
                ...
                player.addModelMessage(new ModelMessage(player, "model.monarch.forceTaxRaise", new String[][] { {
                        "%replace%", String.valueOf(amount) } }, ModelMessage.MessageType.WARNING));
                reply = null;
            } else {
        ...
        case ADD_TO_REF:
            ...
            for (int index = 0; index < childElements.getLength(); index++) {
                ...
                units.add(unit);
                unitNames.add(unit.getNumber() + " " + Messages.getLabel(unit.getUnitType(), unit.getRole()));
            }
        ...
        case DECLARE_WAR:
            ...
            new ShowMonarchPanelSwingTask(action, "%nation%", Messages.getNationAsString(enemy)).confirm();
            break;
        case SUPPORT_LAND:
        case SUPPORT_SEA:
        case ADD_UNITS:
        ...
        case OFFER_MERCENARIES:
            ...
            for (int index = 0; index < childElements.getLength(); index++) {
                AbstractUnit unit = new AbstractUnit();
                unit.readFromXMLElement((Element) childElements.item(index));
                mercenaries.add(unit.getNumber() + " " + Messages.getLabel(unit.getUnitType(), unit.getRole()));
            }
    ...
    private Element setStance(Element element) {
        ...
        if (player.equals(second)) {
            player.addModelMessage(new ModelMessage(first, "model.diplomacy." + stance.toString().toLowerCase()
                    + ".declared", new String[][] { { "%nation%", Messages.getNationAsString(first) } },
                    ModelMessage.MessageType.FOREIGN_DIPLOMACY));
        } else if (stance == Stance.WAR || player.hasAbility("model.ability.betterForeignAffairsReport")
            ...
            player
                    .addModelMessage(new ModelMessage(first, "model.diplomacy." + stance.toString().toLowerCase()
                            + ".others", new String[][] { { "%attacker%", Messages.getNationAsString(first) },
                            { "%defender%", Messages.getNationAsString(second) } },
                            ModelMessage.MessageType.FOREIGN_DIPLOMACY));
        }
        return null;
    }
    ...
    private Element removeGoods(Element element) {
        ...
        if (goodsElement == null) {
            ...
            new ShowModelMessageSwingTask(new ModelMessage(colony, ModelMessage.MessageType.WARNING, null, messageID,
                    "%colony%", colony.getName(), "%amount%", String.valueOf(goods.getAmount()), "%goods%", Messages
                            .getName(goods))).invokeLater();
        }
    ...
    private Element lostCityRumour(Element element) {
        ...
        case BURIAL_GROUND:
            ...
            m = new ModelMessage(unit, "lostCityRumour.BurialGround", new String[][] { { "%nation%",
                    Messages.getNationAsString(indianPlayer) } }, ModelMessage.MessageType.LOST_CITY_RUMOUR);
            break;
        case EXPEDITION_VANISHES:
            m = new ModelMessage(unit, "lostCityRumour.ExpeditionVanishes", null,
        ...
        case NOTHING:
            ...
            break;
        case LEARN:
            m = new ModelMessage(unit, "lostCityRumour.SeasonedScout", new String[][] { { "%unit%",
                    Messages.getLabel(unit) } }, ModelMessage.MessageType.LOST_CITY_RUMOUR);
        ...
        case TREASURE:
            ...
            m = new ModelMessage(unit, ModelMessage.MessageType.LOST_CITY_RUMOUR, newUnit,
                    "lostCityRumour.TreasureTrain", "%money%", treasure);
            player.getHistory().add(
                    new HistoryEvent(player.getGame().getTurn().getNumber(), HistoryEvent.Type.CITY_OF_GOLD,
    ...
    private Element spanishSuccession(Element element) {
        ...
        player.addModelMessage(new ModelMessage(winner, ModelMessage.MessageType.FOREIGN_DIPLOMACY, null,
                "model.diplomacy.spanishSuccession", "%loserNation%", Messages.getNationAsString(loser), "%nation%",
                Messages.getNationAsString(winner)));
        loser.setDead(true);
        update(element);
        player.getHistory().add(
                new HistoryEvent(player.getGame().getTurn().getNumber(), HistoryEvent.Type.SPANISH_SUCCESSION,
                        "%nation%", Messages.getNationAsString(winner), "%loserNation%", Messages
                                .getNationAsString(loser)));
