...
public final class InGameController implements NetworkConstants {
    ...
    /**
     * If true, then at least one unit has been active and the turn may
    ...
    public void saveGame() {
        ...
        String fileName = freeColClient.getMyPlayer().getName() + "_"
                + Messages.getNationAsString(freeColClient.getMyPlayer()) + "_"
                + freeColClient.getGame().getTurn().toSaveGameString();
     ...
     * @param element The <code>Element</code> (root element in a DOM-parsed
    ...
    private Element askExpecting(Client client, Element element, String tag) {
        ...
        if (reply == null) {
            ...
            logger.warning("Received reply" + " with tag " + reply.getTagName() + " which should have been " + tag
                    + " to message " + element);
        }
        return null;
    }
    ...
    public void declareIndependence() {
        ...
        if (player.getSoL() < 50) {
            canvas.showInformationMessage("declareIndependence.notMajority", FreeCol.getSpecification().getGoodsType(
                    "model.goods.bells"), "%percentage%", Integer.toString(player.getSoL()));
            return;
        }
        ...
        if (names == null || names.get(0) == null || names.get(0).length() == 0 || names.get(1) == null
                || names.get(1).length() == 0)
            return;
    ...
    public void sendChat(String message) {
        ChatMessage chatMessage = new ChatMessage(freeColClient.getMyPlayer(), message, false);
    ...
    public void setCurrentPlayer(Player currentPlayer) {
        logger.finest("Entering client setCurrentPlayer(" + currentPlayer.getName() + ")");
        Game game = freeColClient.getGame();
        ...
        logger.finest("Exiting client setCurrentPlayer(" + currentPlayer.getName() + ")");
    }
            ...
            if (savegamePeriod == 1 || (savegamePeriod != 0 && turnNumber % savegamePeriod == 0)) {
                ...
                if (player != null) {
                    playernation = player.getName() + "_"
                        + Messages.getNationAsString(player) + "_";
                }
                String filename = Messages.message("clientOptions.savegames.autosave.fileprefix")
                    + '-' + playernation + freeColClient.getGame().getTurn().toSaveGameString() + ".fsg";
    ...
    public void rename(Nameable object) {
        ...
        if (object instanceof Colony) {
            name = canvas.showInputDialog("renameColony.text", object.getName(), "renameColony.yes", "renameColony.no");
            if (name == null || name.length() == 0) {
                return; // User cancelled
            }
            if (player.getSettlement(name) != null) {
                // Colony name must be unique.
                canvas.showInformationMessage("nameColony.notUnique", "%name%", name);
                return;
            }
        } else if (object instanceof Unit) {
            name = canvas
                    .showInputDialog("renameUnit.text", object.getName(), "renameUnit.yes", "renameUnit.no", false);
            if (name == null) {
        ...
        Element reply = askExpecting(freeColClient.getClient(), message.toXMLElement(), "update");
        if (reply != null) {
    ...
    public void buildColony() {
        ...
        if (build) {
            ...
            String name = canvas.showInputDialog("nameColony.text", player.getDefaultSettlementName(false),
    ...
    private boolean showColonyWarnings(Tile tile, Unit unit) {
        ...
        while (tileIterator.hasNext()) {
            ...
            if (newTile.isLand()) {
                for (Entry<GoodsType, Integer> entry : goodsMap.entrySet()) {
                    entry.setValue(entry.getValue().intValue() + newTile.potential(entry.getKey(), null));
                }
        ...
        if (landLocked) {
            messages.add(new ModelMessage(unit, ModelMessage.MessageType.MISSING_GOODS, FreeCol.getSpecification()
                    .getGoodsType("model.goods.fish"), "buildColony.landLocked"));
        }
        if (food < 8) {
            messages.add(new ModelMessage(unit, ModelMessage.MessageType.MISSING_GOODS, FreeCol.getSpecification()
                    .getGoodsType("model.goods.food"), "buildColony.noFood"));
        }
        for (Entry<GoodsType, Integer> entry : goodsMap.entrySet()) {
            if (!entry.getKey().isFoodType() && entry.getValue().intValue() < 4) {
                messages.add(new ModelMessage(unit, ModelMessage.MessageType.MISSING_GOODS, entry.getKey(),
                        "buildColony.noBuildingMaterials", "%goods%", Messages.getName(entry.getKey())));
            }
        }
        ...
        return freeColClient.getCanvas().showConfirmDialog(modelMessages, "buildColony.yes", "buildColony.no");
    }
    ...
    /**
     * Sets the destination of the given unit and send the server a message for
     * this action.
     *
    ...
    public void moveToDestination(Unit unit) {
        ...
        if (unit.getTradeRoute() != null) {
            ...
            if (!TradeRoute.isStopValid(unit, currStop)) {
                ...
                logger.warning("Trade unit " + unit.getId() + " in route " + oldTradeRouteName
                        + " cannot continue: stop invalid.");
                canvas.showInformationMessage("traderoute.broken", "%name%", oldTradeRouteName);
                clearOrders(unit);
                return;
            }
            ...
            if (unit.getLocation().getTile() == currStop.getLocation().getTile()) {
                ...
                logger.info("Trade unit " + unit.getId() + " in route " + unit.getTradeRoute().getName() + " is at "
                        + unit.getCurrentStop().getLocation());
                followTradeRoute(unit);
                return;
            } else {
                logger.info("Unit " + unit.getId() + " is a trade unit in route " + unit.getTradeRoute().getName()
                        + ", going to " + unit.getCurrentStop().getLocation());
            }
        } else {
            logger.info("Moving unit " + unit.getId() + " to position " + unit.getDestination());
        }
        ...
        if (!(destination instanceof Europe)
                && (destination.getTile() == null || unit.getTile() == destination.getTile())) {
            clearGotoOrders(unit);
            return;
        }
        ...
        if (path == null) {
            canvas.showInformationMessage("selectDestination.failed", unit, "%destination%", Messages
        ...
        while (path != null) {
            ...
            case MOVE_NO_MOVES:
                ...
                return;
            default:
                if (path == path.getLastNode() && mt.isLegal() && (mt != MoveType.ATTACK || knownEnemyOnLastTile(path))) {
                    move(unit, path.getDirection());
    ...
    private boolean knownEnemyOnLastTile(PathNode path) {
        if ((path != null) && path.getLastNode() != null) {
            ...
            return ((tile.getFirstUnit() != null && tile.getFirstUnit().getOwner() != freeColClient.getMyPlayer()) || (tile
                    .getSettlement() != null && tile.getSettlement().getOwner() != freeColClient.getMyPlayer()));
        } else {
            return false;
        }
    }
    ...
    private void followTradeRoute(Unit unit) {
        ...
        if (!TradeRoute.isStopValid(unit, stop)) {
            freeColClient.getCanvas().showInformationMessage("traderoute.broken", "%name%",
        ...
        if (freeColClient.getMyPlayer().getEurope() == stop.getLocation() && !inEurope) {
            return;
        }
        ...
        if (unit.getInitialMovesLeft() == unit.getMovesLeft()) {
            ...
            // moves allowed
            logger.info("Trade unit " + unit.getId() + " in route " + unit.getTradeRoute().getName() + " arrives at "
                    + unit.getCurrentStop().getLocation());
    ...
    private void loadTradeGoodsFromColony(Unit unit) {
        ...
        for (GoodsType goodsType : goodsTypesToLoad) {
            ...
            if (amountPresent > 0) {
                ...
                loadCargo(new Goods(freeColClient.getGame(), location, goodsType, Math.min(amountPresent, 100)), unit);
            } else {
    ...
    private void unloadTradeGoodsToColony(Unit unit) {
        ...
        while (goodsIterator.hasNext()) {
            ...
            if (overflow <= 0 || goods.getType().limitIgnored() || !goods.getType().isStorable()) {
                // Safe to unload the whole load
                all = true;
                logger.finest("Automatically unloading: " + goods + " at " + colonyName);
                ...
                case ClientOptions.UNLOAD_OVERFLOW_RESPONSE_ASK:
                    String msg = Messages.message("traderoute.warehouseCapacity", "%unit%", Messages.getLabel(unit),
                            "%colony%", colonyName, "%amount%", String.valueOf(overflow), "%goods%", Messages
                                    .getName(goods));
                ...
                if (all) {
                    logger.finest("Automatically unloading: " + goods + " at " + colonyName + " overflowing "
                            + Integer.toString(overflow));
                } else {
                    logger.finest("Automatically unloading: " + goods + " at " + colonyName + " retaining "
                            + Integer.toString(overflow));
                }
                if (option != ClientOptions.UNLOAD_OVERFLOW_RESPONSE_ASK) {
                    String whichMessage = (all) ? "traderoute.overflow" : "traderoute.nounload";
                    Player player = freeColClient.getMyPlayer();
                    ModelMessage m = new ModelMessage(player, ModelMessage.MessageType.WAREHOUSE_CAPACITY, player,
                            whichMessage, "%colony%", colonyName, "%unit%", Messages.getLabel(unit), "%overflow%",
                            String.valueOf(overflow), "%goods%", Messages.getName(goods));
    ...
    /**
     * Initiates a negotiation with a foreign power. The player creates a
     * DiplomaticTrade with the NegotiationDialog. The DiplomaticTrade is sent
     * to the other player. If the other player accepts the offer, the trade is
     * concluded. If not, this method returns, since the next offer must come
    ...
    private void negotiate(Unit unit, Direction direction) {
        ...
        Element reply;
        for (;;) {
            newAgreement = canvas.showNegotiationDialog(unit, settlement, oldAgreement);
            if (newAgreement == null) {
                if (oldAgreement != null) {
                    // Inform of rejection of the old agreement
                    message = new DiplomacyMessage(unit, direction, oldAgreement);
                    message.setReject();
            ...
            if (newAgreement.isAccept())
                message.setAccept();
            reply = askExpecting(client, message.toXMLElement(), message.getXMLElementTagName());
            if (reply == null)
                break; // fail
            ...
            if (message.isReject()) {
                ...
                canvas.showInformationMessage("negotiationDialog.offerRejected", "%nation%", nation);
                break;
            } else if (message.isAccept()) {
                String nation = message.getOtherNationName(player);
                canvas.showInformationMessage("negotiationDialog.offerAccepted", "%nation%", nation);
                break;
    ...
    private void exploreLostCityRumour(Unit unit, Direction direction) {
        ...
        Canvas canvas = freeColClient.getCanvas();
        if (canvas.showConfirmDialog("exploreLostCityRumour.text", "exploreLostCityRumour.yes",
    ...
    public boolean claimLand(Tile tile, Colony colony, int offer) {
        ...
        if (price < 0) { // not for sale
            ...
            if (offer >= price || offer < 0) {
                ...
                choices.add(new ChoiceItem<Integer>(Messages.message("indianLand.pay", "%amount%", Integer
                        .toString(price)), CLAIM_ACCEPT, canBuyLand));

                choices.add(new ChoiceItem<Integer>(Messages.message("indianLand.take"), CLAIM_STEAL));

                Integer ci = canvas.showChoiceDialog(Messages.message("indianLand.text", "%player%", Messages
                        .getNationAsString(owner)), Messages.message("indianLand.cancel"), choices);
    ...
    private java.util.Map<String, Boolean> getTransactionSession(Unit unit, Settlement settlement) {
        ...
        Element reply = askExpecting(freeColClient.getClient(), message.toXMLElement(), "getTransactionAnswer");
        if (reply != null) {
     ...
     */
    private List<Goods> getGoodsForSaleInSettlement(Unit unit, Settlement settlement) {
        Game game = freeColClient.getGame();
        GoodsForSaleMessage message = new GoodsForSaleMessage(unit, settlement);
        Element reply = askExpecting(freeColClient.getClient(), message.toXMLElement(), message.getXMLElementTagName());
        if (reply == null) {
    ...
    private void tradeWithSettlement(Unit unit, Direction direction) {
        ...
        if (settlement instanceof IndianSettlement && !((IndianSettlement) settlement).allowContact(unit)) {
            canvas.showInformationMessage("indianSettlement.noContact", settlement);
            return;
        }
        ...
        while ((session = getTransactionSession(unit, settlement)) != null) {
            ...
            if (!buy && !sel && !gif)
                break;
            tradeType = canvas.showIndianSettlementTradeDialog(settlement, buy, sel, gif);
            if (tradeType == null)
                break; // Aborted
            switch (tradeType) {
            case BUY:
    ...
    private void attemptBuyFromSettlement(Unit unit, Settlement settlement) {
        ...
        for (;;) {
            // Choose goods to buy
            goods = canvas.showChoiceDialog(Messages.message("buyProposition.text"), Messages
                    .message("buyProposition.nothing"), goodsOffered);
            if (goods == null)
            ...
            for (;;) {
                ...
                String text = Messages.message("buy.text", "%nation%", Messages
                        .getNationAsString(settlement.getOwner()), "%goods%", Messages.getLabel(goods), "%gold%",
     ...
     * @return The asking price, or NO_TRADE if the trade is outright refused,
     *         or NO_TRADE-1 on error.
     */
    private int proposeToBuyFromSettlement(Unit unit, Settlement settlement, Goods goods, int gold) {
     ...
     */
    private void buyFromSettlement(Unit unit, Settlement settlement, Goods goods, int gold) {
    ...
    private void attemptSellToSettlement(Unit unit, Settlement settlement) {
        ...
        for (;;) {
            ...
            goods = canvas.showChoiceDialog(Messages.message("sellProposition.text"), Messages
                    .message("sellProposition.nothing"), goodsChoices);
            if (goods == null)
                break; // Trade aborted by the player
            ...
            for (;;) {
                ...
                if (gold == NO_NEED_FOR_THE_GOODS) {
                    canvas.showInformationMessage("trade.noNeedForTheGoods", "%goods%", Messages.getName(goods));
                    return;
                ...
                String text = Messages.message("sell.text", "%nation%", Messages.getNationAsString(settlement
                        .getOwner()), "%goods%", Messages.getName(goods), "%gold%", Integer.toString(gold));
                List<ChoiceItem<Integer>> choices = new ArrayList<ChoiceItem<Integer>>();
                ...
                choices.add(new ChoiceItem<Integer>(Messages.message("sell.gift", "%goods%", Messages.getName(goods)),
                        CHOOSE_GIFT));
     ...
     *         NO_TRADE if the trade is outright refused, or NO_TRADE-1 on
     *         error.
     */
    private int proposeToSellToSettlement(Unit unit, Settlement settlement, Goods goods, int gold) {
    ...
    private void attemptGiftToSettlement(Unit unit, Settlement settlement) {
        Canvas canvas = freeColClient.getCanvas();
        Goods goods;
        goods = canvas.showSimpleChoiceDialog(Messages.message("gift.text"), Messages.message("cancel"), unit
                .getGoodsList());
        if (goods != null) {
            deliverGiftToSettlement(unit, settlement, goods);
        }
    ...
    public boolean checkCashInTreasureTrain(Unit unit) {
        ...
        if (europe == null || unit.getLocation() == europe) {
            ...
            String confirm = (unit.getTransportFee() == 0) ? "cashInTreasureTrain.free" : "cashInTreasureTrain.pay";
            cash = canvas.showConfirmDialog(confirm, "cashInTreasureTrain.yes", "cashInTreasureTrain.no");
        }
    ...
    private void reallyMove(Unit unit, Direction direction) {
        ...
        if (unit.getSpaceLeft() > 0 && (unit.getColony() != null || unit.isInEurope())) {
            for (Unit sentry : new ArrayList<Unit>(unit.getLocation().getUnitList())) {
                if (sentry.getState() == UnitState.SENTRY) {
                    if (sentry.getSpaceTaken() <= unit.getSpaceLeft()) {
                        ...
                        logger.finest("Unit " + sentry + " is too large " + "to board " + unit + ": skipped.");
                    }
                }
            }
        }
        ...
        if (!freeColClient.isHeadless()) {
            String key = (freeColClient.getMyPlayer() == unit.getOwner()) ? ClientOptions.MOVE_ANIMATION_SPEED
                    : ClientOptions.ENEMY_MOVE_ANIMATION_SPEED;
            if (freeColClient.getClientOptions().getInteger(key) > 0) {
                Animations.unitMove(canvas, unit, unit.getTile(), game.getMap().getNeighbourOrNull(direction,
                        unit.getTile()));
            }
        }
        ...
        if (unit.getTile().isLand() && !unit.getOwner().isNewLandNamed()) {
            ...
            unit.getOwner().getHistory().add(
                    new HistoryEvent(unit.getGame().getTurn().getNumber(), HistoryEvent.Type.DISCOVER_NEW_WORLD,
                            "%name%", newLandName));

            final Player player = freeColClient.getMyPlayer();
            final BuildColonyAction bca = (BuildColonyAction) freeColClient.getActionManager().getFreeColAction(
            ...
            player.addModelMessage(new ModelMessage(player, ModelMessage.MessageType.TUTORIAL, player,
                                                    ...
                                                    "%build_colony_menu_item%", Messages.message("buildColonyAction.name"),
        ...
        if (region != null) {
            ...
            if (region.isPacific()) {
                ...
                name = freeColClient.getCanvas().showInputDialog("nameRegion.text", defaultName, "ok", "cancel",
                        "%name%", region.getDisplayName());
                moveElement.setAttribute("regionName", name);
            }
            if (name != null) {
                freeColClient.getMyPlayer().getHistory().add(
                        new HistoryEvent(freeColClient.getGame().getTurn().getNumber(),
                                HistoryEvent.Type.DISCOVER_REGION, "%region%", name));
            }
        }
        ...
        if (reply.hasAttribute("movesSlowed")) {
            ...
            canvas.showInformationMessage("model.unit.slowed", slowedBy, "%unit%", Messages.getLabel(unit),
                    "%enemyUnit%", Messages.getLabel(slowedBy), "%enemyNation%", Messages.getNationAsString(slowedBy
                            .getOwner()));
        }
        ...
        if (unit.getTile().getSettlement() != null && unit.isCarrier() && unit.getTradeRoute() == null
            ...
            if (freeColClient.getClientOptions().getBoolean(ClientOptions.UNIT_LAST_MOVE_DELAY)) {
                ...
                try {
                    // UNIT_LAST_MOVE_DELAY is an instance variable located at
            ...
            case INDIAN_SETTLEMENT_TRIBUTE:
                ...
                if (reply != null && reply.getTagName().equals("armedUnitDemandTributeResult")) {
                    ...
                    if (result.equals("agree")) {
                        ...
                        freeColClient.getCanvas().updateGoldLabel();
                        freeColClient.getCanvas().showInformationMessage("scoutSettlement.tributeAgree", settlement,
                                "%replace%", amount);
    ...
    private boolean confirmHostileAction(Unit attacker, Tile target) {
        ...
        case PEACE:
            return freeColClient.getCanvas().showConfirmDialog("model.diplomacy.attack.peace",
                    "model.diplomacy.attack.confirm", "cancel", "%nation%", Messages.getNationAsString(enemy));
        case WAR:
    ...
    private boolean confirmPreCombat(Unit attacker, Tile target) {
        if (freeColClient.getClientOptions().getBoolean(ClientOptions.SHOW_PRECOMBAT)) {
            ...
            Canvas canvas = freeColClient.getCanvas();
            return canvas.showFreeColDialog(new PreCombatDialog(attacker, defenderOrNull, settlementOrNull, canvas));
        }
        return true;
    }
    ...
    private void reallyAttack(Unit unit, Direction direction) {
        ...
        if (attackResultElement != null && attackResultElement.getTagName().equals("attackResult")) {
            ...
            Location repairLocation = (Location) game.getFreeColGameObjectSafely(attackResultElement
                    .getAttribute("repairIn"));
            ...
            } else if (defender.isNaval() && result == CombatResultType.GREAT_WIN || unit.isNaval()
            ...
            if (convertElement != null) {
                ...
                ModelMessage message = new ModelMessage(convert, "model.unit.newConvertFromAttack", new String[][] {
                        { "%nation%", nation }, { "%unit%", Messages.getLabel(convert) } },
                        ModelMessage.MessageType.UNIT_ADDED);
                freeColClient.getMyPlayer().addModelMessage(message);
                nextModelMessage();
            }

            if (defender.canCarryTreasure() && (result == CombatResultType.WIN || result == CombatResultType.GREAT_WIN)) {
            ...
            if (attackResultElement.getAttribute("indianCapitalBurned") != "") {
                ...
                // show message
                ModelMessage message = new ModelMessage(indianPlayer, "indianSettlement.capitalBurned", new String[][] {
    ...
    private void embark(Unit unit, Direction direction) {
        ...
        if (destinationTile.getUnitCount() == 1) {
            ...
            if (choices.size() == 1) {
                ...
                destinationUnit = canvas.showSimpleChoiceDialog(Messages.message("embark.text"), Messages
                        .message("embark.cancel"), choices);
    ...
    public void clearSpeciality(Unit unit) {
        if (freeColClient.getGame().getCurrentPlayer() != freeColClient.getMyPlayer()) {
            ...
            if (newUnit == null) {
                freeColClient.getCanvas().showInformationMessage("clearSpeciality.impossible", "%unit%",
                        Messages.getLabel(unit));
                return;
            } else if (!freeColClient.getCanvas().showConfirmDialog("clearSpeciality.areYouSure", "yes", "no",
                    "%oldUnit%", Messages.getLabel(unit), "%unit%", Messages.getName(newUnit))) {
                return;
            }
        }
    ...
    /**
     * Unload cargo. If the unit carrying the cargo is not in a harbour, the
     * goods will be dumped.
     *
    ...
    public void setGoodsLevels(Colony colony, GoodsType goodsType) {
        ...
        setGoodsLevelsElement.appendChild(data
                .toXMLElement(colony.getOwner(), setGoodsLevelsElement.getOwnerDocument()));
    ...
    public void work(Unit unit, WorkLocation workLocation) {
        ...
        if ((tile.getOwner() != unit.getOwner() || tile.getOwningSettlement() != workLocation.getColony())
                && !claimLand(tile, workLocation.getColony(), 0)) {
            logger.warning("Unit " + unit.getId() + " is unable to claim tile " + tile.toString());
            return;
        }
    ...
    public boolean putOutsideColony(Unit unit) {
        ...
        if (reply != null && reply.getTagName().equals("update")) {
            ...
            unit.getColony().firePropertyChange(Colony.ColonyChangeEvent.POPULATION_CHANGE.toString(), oldPopulation,
                    unit.getColony().getUnitCount());
            unit.getTile().firePropertyChange(Tile.UNIT_CHANGE, null, unit);
        } else {
    ...
    public void changeWorkImprovementType(Unit unit, TileImprovementType improvementType) {
        ...
        if (!improvementType.isNatural() && freeColClient.getMyPlayer() != unit.getTile().getOwner()
                && !claimLand(unit.getTile(), null, 0)) {
            logger.warning("Unit " + unit.getId() + " is unable to claim tile " + unit.getTile().toString());
            return;
        }
        ...
        if (containerElement != null) {
            TileItemContainer container = (TileItemContainer) freeColClient.getGame().getFreeColGameObject(
        ...
        if (improvementElement != null) {
            TileImprovement improvement = (TileImprovement) freeColClient.getGame().getFreeColGameObject(
                    improvementElement.getAttribute("ID"));
            if (improvement == null) {
    ...
    public void changeState(Unit unit, UnitState state) {
        ...
        if (state == UnitState.FORTIFYING && unit.isOffensiveUnit() && !unit.hasAbility("model.ability.piracy")) { // check
            // if
            // it's
            // going
            // to
            ...
            // tile
            Tile tile = unit.getTile();
            if (tile != null && tile.getOwningSettlement() != null) { // check
                // stance
                // with
                // settlement's
                // owner
                ...
                if (myPlayer != enemy && myPlayer.getStance(enemy) != Stance.ALLIANCE
                        && !confirmHostileAction(unit, tile.getOwningSettlement().getTile())) { // player
                    // has
                    // aborted
                    return;
                }
        ...
        if (!freeColClient.getCanvas().isShowingSubPanel()
                && (unit.getMovesLeft() == 0 || unit.getState() == UnitState.SENTRY || unit.getState() == UnitState.SKIPPED)) {
            nextActiveUnit();
        } else {
            freeColClient.getCanvas().refresh();
        }
     ...
     * by making the unit {@link UnitState#ACTIVE} and setting a null
     * destination
     *
     * @param unit The <code>Unit</code>.
     */
    public void clearOrders(Unit unit) {
        ...
        if (unit.getState() == UnitState.IMPROVING) {
            ...
            boolean cancelWork = freeColClient.getCanvas().showConfirmDialog(new ModelMessage[] { message }, "yes",
                    "no");
            if (!cancelWork) {
                return;
            }
    ...
    private void learnSkillAtIndianSettlement(Unit unit, Direction direction) {
        ...
        if (settlement != null) {
            ...
            if (skill == null) {
                ...
                canvas.showInformationMessage("indianSettlement.cantLearnSkill", settlement, "%unit%", Messages
                        .getLabel(unit), "%skill%", Messages.getName(skill));
            } else {
                ...
                if (!canvas.showConfirmDialog("learnSkill.text", "learnSkill.yes", "learnSkill.no", "%skill%", Messages
                        .getName(skill))) {
    ...
    /**
     * Ask for spy the foreign colony, negotiate with the foreign power or
     * attack the colony
     *
    ...
    private void scoutIndianSettlement(Unit unit, Direction direction) {
        ...
        if (reply.getTagName().equals("scoutIndianSettlementResult")) {
            ...
            settlement.setLearnableSkill(skill);
            settlement.setWantedGoods(0, FreeCol.getSpecification().getGoodsType(
                    reply.getAttribute("highlyWantedGoods")));
        ...
        case INDIAN_SETTLEMENT_ATTACK:
            ...
            if (confirmPreCombat(unit, tile)) {
                ...
                // The player chose to not attack, so the scout shouldn't get
                // back his moves
                unit.setMovesLeft(0);
            }
            return;
        ...
        if (reply.getTagName().equals("scoutIndianSettlementResult")) {
            ...
            if (result.equals("die")) {
                ...
                if (result.equals("tales")) {
                    ...
                    unit.getOwner().modifyGold(Integer.parseInt(amount));
                    freeColClient.getCanvas().updateGoldLabel();
                    canvas.showInformationMessage("scoutSettlement.speakBeads", settlement, "%amount%", amount);
                    ...
                    canvas.showInformationMessage("scoutSettlement.expertScout", settlement, "%unit%", Messages
                            .getName(unit.getType()));
                }
    ...
    public void payForBuilding(Colony colony) {
        ...
        if (!freeColClient.getCanvas().showConfirmDialog("payForBuilding.text", "payForBuilding.yes",
                "payForBuilding.no", "%replace%", Integer.toString(colony.getPriceForBuilding()))) {
            return;
        }
    ...
    public void setTradeRoutes(List<TradeRoute> routes) {
        ...
        for (TradeRoute route : routes) {
            Element routeElement = tradeRoutesElement.getOwnerDocument().createElement(
                    TradeRoute.getXMLElementTagName());
    ...
    public void payArrears(GoodsType type) {
        ...
        if (player.getGold() >= arrears) {
            if (freeColClient.getCanvas().showConfirmDialog("model.europe.payArrears", "ok", "cancel", "%replace%",
            ...
            freeColClient.getCanvas().showInformationMessage("model.europe.cantPayArrears", "%amount%",
                    String.valueOf(arrears));
        }
    }
    ...
    public void abandonColony(Colony colony) {
        ...
        colony.getOwner().getHistory().add(
                new HistoryEvent(colony.getGame().getTurn().getNumber(), HistoryEvent.Type.ABANDON_COLONY, "%colony%",
                        colony.getName()));
