...
eval e (p :<->: q)     =  (eval e (Not p) || eval e q) && (eval e (Not q) || eval e p)
...
-- 2. 
tautology :: Prop -> Bool
tautology ps = foldr (&&) True ([eval x ps | x <- (envs (names ps))])
...
equivalent ps qs = and ([eval x (ps :<->: qs) | x <- (envs (names (ps :<->: qs)))])

equivalent' :: Prop -> Prop -> Bool
equivalent' ps qs =  tautology (ps:<->:qs)

prop_equivalent :: Prop -> Prop -> Bool
prop_equivalent qs xs = equivalent' qs xs == equivalent qs xs


-- 5.
subformulas :: Prop -> [Prop]
--subformulas [] = [[]]
--subformulas (x:xs) = subformulas xs ++ [x:ys | ys <- subformulas xs]
subformulas p = nub( p : sub p)
                where  
                  sub (Var x) = []
                  sub T = []
                  sub (F) = []
                  sub (Not p) = subformulas p
                  sub (p :|: q) =  subformulas p ++ subformulas  q
                  sub (p :->: q) = subformulas  p ++ subformulas q
                  sub (p :&: q) = subformulas p ++ subformulas q
                  sub (p :<->: q) = subformulas p ++ subformulas q

subformulas2 :: Prop -> [[Prop]]
subformulas2 p = subs(subformulas p)
...
--prop_NNF1 :: Prop -> Bool
--prop_NNF1 p  = (isNNF (toNNF p)->:Var "Q"):&:(Var "P" :&: Not(Var("Q")))
...
--toCNF :: Prop -> Prop
--toCNF p  =  listsToCNF (toCNFList p)->:Var "Q"):&:(Var "P" :&: Not(Var("Q"))
