...
class Vector3f {
	...
	Vector3f cross(const Vector3f & input) const
	{
		float c1 = (_item[1] * input._item[2] - _item[2] * input._item[1]);
		float c2 = (_item[2] * input._item[0] - _item[0] * input._item[2]);
		float c3 = (_item[0] * input._item[1] - _item[1] * input._item[0]);
...
class Matrix4f {
	...
	void createRotX(float rad)
	{
		setIdentity();
		float s = (float)sin(rad);
		_item[1][1] = _item[2][2] = (float)cos(rad);

		_item[2][1] = s;
		_item[1][2] = -s;
	}

	void createRotY(float rad)
	{
		setIdentity();
		float s = (float)sin(rad);
		_item[0][0] = _item[2][2] = (float)cos(rad);
		_item[0][2] = s;
		_item[2][0] = -s;
	}

	void createRotZ(float rad)
	{
		setIdentity();
		float s = (float)sin(rad);
		_item[0][0] = _item[1][1] = (float)cos(rad);
		_item[1][0] = s;
		_item[0][1] = -s;
	}
